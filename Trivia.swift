...
class Trivia {
    ...
    //note: it's static, so it will be there for every trivia obj. access with Trivia.questionsDB
    ...
    static func getCategories() -> [String] {
        return Array(Trivia.questionsDB.keys) as [String]
    }
    static func getQuestionsOfCategory(categoryName: String) throws -> [Trivia] {
        guard Trivia.questionsDB.keys.contains(categoryName) else {
            throw TriviaClassError.categoryNotFound
        }
        return Trivia.questionsDB[categoryName]!
    }
    
    static func getQuestionCountofCategory(categoryName: String) throws -> Int {
        guard Trivia.questionsDB.keys.contains(categoryName) else {
            throw TriviaClassError.categoryNotFound
        }
        return Trivia.questionsDB[categoryName]!.count
    }
    
    //ERROR handling
    //trying this out using tutorial from: https://docs.swift.org/swift-book/documentation/the-swift-programming-language/errorhandling/
    enum TriviaClassError: Error {
